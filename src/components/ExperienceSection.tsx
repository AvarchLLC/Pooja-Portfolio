    ...
    <section id="experience" className="py-28 relative" ref={ref}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="relative">
          ...
          <motion.div
            className="absolute left-[15px] md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-accent via-accent-cyan to-accent-warm"
            initial={{ scaleY: 0 }}
          ...
          <div className="space-y-8">
            {experience.map((exp, i) => (
              ...
              >
                {/* Timeline dot */}
                <motion.div
                  className="absolute left-[15px] md:left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-gradient-primary z-10 mt-6 md:mt-8 ring-4 ring-background"
                  initial={{ scale: 0 }}
                  animate={inView ? { scale: 1 } : {}}
                  transition={{ duration: 0.4, delay: 0.2 + i * 0.07, type: "spring" }}
                />
                ...
                <div className={`ml-10 md:ml-0 md:w-1/2 ${i % 2 === 0 ? "md:pr-10" : "md:pl-10"}`}>
                  <motion.div
                    ...
                    whileHover={{ y: -5, boxShadow: "0 20px 40px -15px hsl(280 80% 65% / 0.15)", transition: { duration: 0.3 } }}
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="text-lg font-bold text-foreground">{exp.role}</h3>
                      {exp.highlight && (
                        <motion.span
                          className="text-[10px] px-2 py-0.5 rounded-full bg-gradient-primary text-primary-foreground font-semibold flex-shrink-0"
